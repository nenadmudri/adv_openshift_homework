...
oc patch route/mlbparks -p '{"spec":{"to":{"name":"g-mlbparks"}}}' -n $GUID-parks-prod 
oc patch route/nationalparks -p '{"spec":{"to":{"name":"g-nationalparks"}}}' -n $GUID-parks-prod 
oc patch route/parksmap   -p '{"spec":{"to":{"name":"g-parksmap"}}}' -n $GUID-parks-prod 
